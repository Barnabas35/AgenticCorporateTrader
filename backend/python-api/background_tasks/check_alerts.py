            ...
            if market == "stocks":
                ...
                if previous_ticker_price is not None:
                    if (previous_ticker_price <= price <= current_ticker_price) or (previous_ticker_price >= price >= current_ticker_price):
                        ...
                        # Send email to user
                        send_email_smtp(user_email, f"Price Alert For {ticker}", f"Your price alert for the stock {ticker} set at ${price} has been triggered. The price is now ${current_ticker_price}.")
                ...
                if previous_ticker_price is not None:
                    if (previous_ticker_price <= price <= current_ticker_price) or (previous_ticker_price >= price >= current_ticker_price):
                        ...
                        # Send email to user
                        send_email_smtp(user_email, f"Price Alert For {ticker}", f"Your price alert for the crypto {ticker} set at ${price} has been triggered. The price is now ${current_ticker_price}.")

        previous_ticker_prices_stocks = current_ticker_prices_stocks
        previous_ticker_prices_crypto = current_ticker_prices_crypto

        print(previous_ticker_prices_stocks)
        print(previous_ticker_prices_crypto)
